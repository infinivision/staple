...
void fhog28(cv::MatND &fhog_feature, cv::MatND &fhog_feature2, const cv::Mat& input, int binSize, int nOrients, float clip, bool crop) {
    int HEIGHT = input.rows;
    int WIDTH = input.cols;
    int DEPTH = input.channels();
    ...
    /*
    // [to be delete
    fhog_feature.create(h,w,CV_32FC(CHANNELS));
    float* H = (float *) fhog_feature.data;
