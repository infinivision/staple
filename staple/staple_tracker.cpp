...
string STAPLE_TRACKER::wisdomFile = "wisdom";
...
void inline STAPLE_TRACKER::fftwInit(int row, int col, int cn) {
    ...
    xtfv.resize(cn);
    xtfrv.resize(cn);
    xtfv_windows.resize(cn);

    float * p  = (float *)fftwf_mallocWrapper(sizeof(float) * row * col * cn);
    xt2 = cv::Mat(cn,row*col,CV_32FC1,(void *)p);

    fftwf_complex * p2 = (fftwf_complex *)fftwf_mallocWrapper(sizeof(fftwf_complex) * row * (col/2+1) * cn);
    xtf2  =  cv::Mat(cn,row*(col/2+1),CV_32FC2,(void *)p2);

    xtfr2.create(cn,row*(col/2+1),CV_32FC1);
    float * p3 = (float *) xtfr2.data;

    fftwf_complex * p4 = (fftwf_complex *)fftwf_mallocWrapper(sizeof(fftwf_complex) * row * (col/2+1) * cn);
    xtf2_windows  =  cv::Mat(cn,row*(col/2+1),CV_32FC2,(void *)p4);

    for(int i = 0; i< cn ; i++){
      xtv[i]    = cv::Mat(row,col,CV_32FC1,(void *)(p+i*row*col));
      xtfv[i]   = cv::Mat(row,(col/2+1),CV_32FC2,(void *)(p2+i*row*(col/2+1)));
      xtfrv[i]  = cv::Mat(row,(col/2+1),CV_32FC1,(void *)(p3+i*row*(col/2+1)));
...
*/
void inline STAPLE_TRACKER::fftTool(const cv::Mat & src, cv::Mat & dest) {
    assert(src.channels() == 1);
    fftwf_execute_dft_r2c(fftPlan, (float *) src.data, (fftwf_complex *) dest.data);
}

void inline STAPLE_TRACKER::fftTool(const vector<cv::Mat> & src, std::vector<cv::Mat> & dest) {
    for(int i=0;i<src.size();i++)
        fftTool(src[i],dest[i]);
}

void inline STAPLE_TRACKER::ifftTool(const cv::Mat & src, cv::Mat & dest) {
    assert(src.channels() == 2);
    fftwf_execute_dft_c2r(ifftPlan, (fftwf_complex *) src.data, (float *) dest.data);
}

void inline STAPLE_TRACKER::sumChannels(std::vector<cv::Mat> & src, cv::Mat & dest, bool real) {
    if(real){
        dest=src[0].clone();
    } else {
        src[0].copyTo(dest);
    ...
    if(real){
        Map<ArrayXf> a((float*) dest.data ,len);
        for(unsigned i=1;i<src.size();i++){
            Map<ArrayXf> b((float *) src[i].data,len);
            a = a + b;
        }
    } else{
...
void STAPLE_TRACKER::getFeatureMap(cv::Mat &im_patch, const char *feature_type, cv::MatND &output)
{
...
#else
    fhog28(output, xt2, im_patch, cfg.hog_cell_size, 9);
#endif
    ...
    */
    // add eigen fftw
    Map<ArrayXf>  a_hann_window((float *) hann_window.data, hann_window.rows*hann_window.cols);
    Map<ArrayXf>  a_grayimgf((float *) grayimgf.data, grayimgf.rows*grayimgf.cols);
    for(size_t i=1;i<xtv.size();i++){
        Map<ArrayXf>  a_xtv((float *) xtv[i].data, xtv[0].rows*xtv[0].cols);
        a_xtv *= a_hann_window;
    }

    Map<ArrayXf>  a_xt2((float *) xt2.data,xt2.cols);
    a_xt2 = a_grayimgf * a_hann_window;
...
void STAPLE_TRACKER::getScaleSubwindow(const cv::Mat &im, cv::Point_<float> centerCoor, cv::Mat &output)
{
    ...
    cv::Mat im_patch_resized_tmp1;
    cv::Mat im_patch_resized_tmp2;
    cv::MatND temp;

    cv::Size_<float> last_sz;
    ...
    // add eigen fftw
{
    ...
    float area = cf_response_size.width*cf_response_size.height;
    int len  = xtfv[0].rows*(xtfv[0].cols);
    int clen = xtfv.size() * len;
    
    // calaculate magnitude of complex feature
    Map<ArrayXcf> a1((std::complex<float> *) xtf2.data, clen);
    Map<ArrayXf>  a2((float*) xtfr2.data, clen);
    a2 = a1.abs2() / area;

    // every channel complext conjugate multiply
    Map<ArrayXcf> a3((std::complex<float> *) yf_fftw.data ,len);
    for(size_t i =0;i<xtfv.size();i++){
      Map<ArrayXcf> a4((std::complex<float> *) xtfv[i].data,len);
      a4 *= a3.conjugate()/area;
    }
    ...
    if(first){
        ...
        a1 = (1 - cfg.learning_rate_cf)*a1_old + cfg.learning_rate_cf*a1;
        a2 = (1 - cfg.learning_rate_cf)*a2_old + cfg.learning_rate_cf*a2;
...
cv::Rect STAPLE_TRACKER::tracker_staple_update(const cv::Mat &im)
{
    // extract patch of size bg_area and resize to norm_bg_area
    //cv::Mat im_patch_cf;
    cv::Mat im_patch_cf = im_patch_bg;
...
{
    // add for eigen fftw
    fftTool(xtv,xtfv_windows);
    float area = cf_response_size.width*cf_response_size.height;
    int len  = xtfv[0].rows*xtfv[0].cols;
    int clen = xtfv.size() * len;

    sumChannels(xtfrv,xtfr_sum,true);

    Map<ArrayXf>  a1((float*) xtfr_sum.data, len);
    a1 += cfg.lambda;

    for(size_t i =0;i<xtfv.size();i++){
      Map<ArrayXcf> a2((std::complex<float> *) xtfv[i].data ,len);
      a2 /= a1;
    }

    Map<ArrayXcf>  a3((std::complex<float> *) xtf2.data, clen);
    Map<ArrayXcf>  a4((std::complex<float> *) xtf2_windows.data, clen);
    a4 *= a3.conjugate();

    sumChannels(xtfv_windows, response_cff_fftw ,false);

    //cout << "response_cff_fftw.col(0)-response_cff.col(0): " << norm(response_cff_fftw.col(0)-response_cff.col(0)) << endl;

    ifftTool(response_cff_fftw,response_cf_fftw);

    Map<ArrayXf>  a5((float*) response_cf_fftw.data,response_cf_fftw.rows*response_cf_fftw.cols );
    a5 /= area ;

}
    // cout << "response_cf norm: " << norm(response_cf) << endl;
    // cout << "response_cf dif norm: " <<norm(response_cf-response_cf_fftw) << endl;
    ...
    cv::Mat response_cf;
    cropFilterResponse(response_cf_fftw, newsz, response_cf);
